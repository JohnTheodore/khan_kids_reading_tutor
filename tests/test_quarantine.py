...
from khan_kids.quarantine import append_low_score_quarantines, read_active_quarantines
...
class QuarantineTests(unittest.TestCase):
    ...
    def test_fourth_attempt_below_70_starts_a_fourteen_day_quarantine(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            path = Path(temporary) / "quarantines.csv"
            active = append_low_score_quarantines(
                path,
                student="Student A",
                today=date(2026, 9, 12),
                scores={("Words: End Sound", "Main"): (83, 56, 68, 69)},
                new_attempts=(
                    {
    ...
    def test_latest_score_of_70_does_not_trigger_quarantine(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            active = append_low_score_quarantines(
                ...
                student="Student A",
                today=date(2026, 9, 12),
                scores={("Words: End Sound", "Main"): (83, 56, 68, 70)},
                new_attempts=(
                    {
                        "lesson_title": "Words: End Sound",
                        "activity_variant": "Main",
