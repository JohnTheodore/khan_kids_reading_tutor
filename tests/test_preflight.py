...
from khan_kids.adb import LOCK_TASK_NONE, LOCK_TASK_PINNED, AndroidDevice, AutomationError
...
class TabletPreflightTests(unittest.TestCase):
    ...
    def test_preflight_stops_before_app_navigation_when_offline(self) -> None:
        ...
        device.lock_task_mode.return_value = LOCK_TASK_NONE
    ...
    def test_screen_pinning_stops_before_network_or_app_navigation(self) -> None:
        device = Mock(spec=AndroidDevice)
        device.is_locked.return_value = False
        device.lock_task_mode.return_value = LOCK_TASK_PINNED

        with self.assertRaisesRegex(AutomationError, "screen-pinned"):
            assert_tablet_preflight(device)

        device.command.assert_not_called()
        health = tablet_health(device)
        self.assertFalse(health["ready"])
        self.assertEqual(health["lock_task_mode"], LOCK_TASK_PINNED)
        self.assertEqual(health["checks"][-1]["id"], "screen_pinning")
